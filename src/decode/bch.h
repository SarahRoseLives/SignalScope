/*
 * bch.h - BCH(31,21,2) Error Correction for FLEX and POCSAG (header-only C++17)
 *
 ...
 *
 * Ported from multimon-ng bch.c to header-only C++17 for PagerScope.
 * Same algorithms, same tables, same GF(2^5) math.
 */
#pragma once
#include <cstdint>
#include <cstring>
...
/* ---------- forward declaration so encode/correct can lazy-init ---------- */
inline void bch_init();

/* GF(2^5) field tables - computed at init */
inline unsigned char flex_exp_tbl[32];      /* alpha^i -> polynomial representation */
inline unsigned char flex_log_tbl[32];      /* inverse: polynomial -> exponent */

/* Generator polynomial for FLEX BCH - coefficients are GF(2^5) elements */
inline unsigned char flex_gen_poly[BCH_PARITY_BITS + 1];

/* Syndrome tables for FLEX - computed at init from exp table */
inline unsigned char flex_s1_tbl[BCH_CODE_LEN];  /* alpha^i for syndrome S1 */
inline unsigned char flex_s3_tbl[BCH_CODE_LEN];  /* alpha^(3i) for syndrome S3 */

/*
 * FLEX parity table - computed at init from generator polynomial.
 * flex_parity_tbl[i] = 10-bit parity when only data bit i is set.
 */
inline unsigned short flex_parity_tbl[BCH_DATA_BITS];
 ...
 */
inline unsigned short pocsag_parity_tbl[BCH_DATA_BITS];

/* Error correction lookup tables - computed at init */
inline unsigned int flex_err_tbl[1024];     /* (S1<<5)|S3 -> error pattern */
inline unsigned int pocsag_err_tbl[2048];   /* syndrome -> error pattern */

/* Single-bit syndrome tables for efficient table building and syndrome computation */
inline unsigned int flex_bit_key[BCH_CODE_LEN];  /* (S1<<5)|S3 for single bit */
inline unsigned short pocsag_syn_tbl[32];        /* 10-bit BCH syndrome for single bit */

inline int bch_initialized = 0;

/* ========== Utility Functions ========== */

inline unsigned int bch_parity32(unsigned int x)
{
...
inline int bch_popcount32(unsigned int x)
{
#if defined(__GNUC__) || defined(__clang__)
...
inline int bch_ctz32(unsigned int x)
{
#if defined(__GNUC__) || defined(__clang__)
    ...
    return debruijn32[((x & (0u - x)) * 0x077CB531u) >> 27];
#endif
}
 ...
 *
 * recv[i] corresponds to bit (30-i) of the uint32_t.
 */

/* Compute FLEX syndrome key (S1<<5)|S3 using GF(2^5) field tables */
inline unsigned int flex_syndrome_key(unsigned int codeword)
{
    unsigned int s1 = 0, s3 = 0;

    // Mask to 31 bits: FLEX BCH(31,21) codewords use only bits 0-30.
    // Bit 31 is a control flag, not part of the BCH data.
    codeword &= 0x7FFFFFFF;

    /* Iterate only over set bits for efficiency */
    while (codeword) {
        int bit = bch_ctz32(codeword);
        int recv_idx = 30 - bit;
...
/*
 * Encode 21-bit data into a 31-bit FLEX codeword (in place).
 * Input:  *data in bits 0-20
 * Output: *data = 31-bit codeword (data in bits 0-20, parity in bits 21-30)
 * Returns: 0 on success.
 */
inline int bch_flex_encode(uint32_t* data)
{
    if (!bch_initialized)
        bch_init();
    ...
    *data = (*data & 0x1FFFFF) | (parity << BCH_DATA_BITS);
    return 0;
}
 ...
 */
inline int bch_flex_correct(uint32_t* codeword)
{
    if (!bch_initialized)
        bch_init();
...
inline unsigned int pocsag_syndrome(unsigned int codeword)
{
    unsigned int syn = 0;
...
/*
 * Encode 21-bit data into a 32-bit POCSAG codeword (in place).
 * Input:  *data in bits 0-20
 * Output: *data = 32-bit codeword (data in bits 31-11, parity in bits 10-1, even parity in bit 0)
 * Returns: 0 on success.
 */
inline int bch_pocsag_encode(uint32_t* data)
{
    if (!bch_initialized)
        bch_init();
    ...
    codeword |= bch_parity32(codeword);  /* Even parity in bit 0 */
 ...
 * Returns: 0 = no errors, 1-2 = corrected bit count, -1 = uncorrectable
 */
inline int bch_pocsag_correct(uint32_t* codeword)
{
    if (!bch_initialized)
        bch_init();
...
inline void build_gf_tables()
{
    /* Build exp table: alpha^i -> polynomial representation */
    unsigned int elem = 1;
    for (int i = 0; i < BCH_CODE_LEN; i++) {
        flex_exp_tbl[i] = (unsigned char)elem;
        flex_log_tbl[elem] = (unsigned char)i;
        elem <<= 1;
 ...
 */
inline void build_generator_poly()
{
    int seen[32] = {0};
 ...
 */
inline void build_flex_parity_table()
{
    for (int databit = 0; databit < BCH_DATA_BITS; databit++) {
            ...
            if (feedback != 0) {
                for (int j = BCH_PARITY_BITS - 1; j > 0; j--) {
                    if (flex_gen_poly[j] != 0)
                        bb[j] = bb[j - 1] ^ (unsigned char)feedback;
                    else
                        bb[j] = bb[j - 1];
                }
                bb[0] = (unsigned char)(flex_gen_poly[0] && feedback);
            } else {
                for (int j = BCH_PARITY_BITS - 1; j > 0; j--) {
                    bb[j] = bb[j - 1];
                }
        ...
        flex_parity_tbl[databit] = (unsigned short)parity;
    }
}

inline void bch_init()
{
    static std::once_flag g_bchInitOnce;
    std::call_once(g_bchInitOnce, []() {
        if (bch_initialized)
            return;
    ...
    for (int databit = 0; databit < BCH_DATA_BITS; databit++) {
        ...
        pocsag_parity_tbl[databit] = (unsigned short)(shreg & 0x3FF);
    }

    /* ===== Clear error lookup tables ===== */
    for (unsigned int& v : flex_err_tbl) v = 0;
    for (unsigned int& v : pocsag_err_tbl) v = 0;
    ...
    for (int bit = 0; bit < 31; bit++) {
        ...
        pocsag_syn_tbl[bit] = (unsigned short)(shreg & 0x3FF);
    }

    /* ===== POCSAG: Build error correction table ===== */
    /* Single-bit errors (bits 1-31, not bit 0 which is parity) */
    for (int i = 1; i < 32; i++) {
        /* Single bit always causes parity error */
        unsigned int syn = (unsigned int)pocsag_syn_tbl[i - 1] | 0x400;
        pocsag_err_tbl[syn] = 1u << i;
    }
    /* Two-bit errors: XOR the single-bit syndromes (parity cancels out) */
    for (int i = 1; i < 32; i++) {
        for (int j = i + 1; j < 32; j++) {
            unsigned int syn = (unsigned int)pocsag_syn_tbl[i - 1] ^ pocsag_syn_tbl[j - 1];
