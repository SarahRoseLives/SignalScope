...
#endif
#ifdef HAS_LIBRESDR
    WD(libreSampleRateMHz); WI(libreSampleRateIdx); WF(libreGainDb); WI(libreAntennaIdx);
    WS(libreFpgaPath);
#endif
...
#endif
#ifdef HAS_LIBRESDR
    RD(libreSampleRateMHz); RI(libreSampleRateIdx); RF(libreGainDb); RI(libreAntennaIdx);
    RS(libreFpgaPath);
#endif
...
void cfgRegisterHandler(App& app)
{
    ...
    h.TypeName = "SignalScope";
    h.TypeHash = ImHashStr("SignalScope");
    h.UserData = &app;
